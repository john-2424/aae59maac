...
twenty-five held-out graphs, which is a $1.31\times$ lift over the
...
SDP bound no longer applies, it also outperforms a collision-aware
centroid heuristic while keeping the agents reasonably spread out. A
...
max, min) of $L(w_k)$, (ii) the top-$\kappa$ Laplacian eigenvalues
with $\kappa = 4$, (iii) the current weight vector $w_k$ padded out
to $m_{\max}$, (iv) the normalized remaining budget, and (v) an
exponential moving average of $\log J_{\text{dis}}$. The action
$a_k \in [-1,1]^m$ is mapped to weights through
...
reaches the same threshold in $7$ steps. We also recover the
closed-form ring eigenvalue $\lambda_2 = 2(1 - \cos(2\pi/n))$ and
the complete-graph eigenvalue $\lambda_2 = n$ to machine precision,
which gives us reasonable confidence the pipeline is working as
intended.
...
and a tight budget $B = 25$. The budget is set below the expected
sum of Metropolis weights, so the policy actually has to choose
which edges to put weight on. Five seeds are trained independently,
and each one is evaluated on five held-out graphs from the same
distribution, giving 25 independent held-out evaluations in total.
...
\label{sec:sim-m3b}
To probe robustness, we evaluate two policies under i.i.d.\
Bernoulli edge failures at rates $p \in \{0, 0.1, 0.2, 0.3\}$:
(i) the clean-trained PPO policy from Section~\ref{sec:sim-m2}, and
(ii) a PPO policy trained with perturbation-in-the-loop at $p =
0.15$. Figure~\ref{fig:m3b} overlays the $\lambda_2$ curves for
both variants alongside uniform and Metropolis.
...
associated mean minimum pairwise distance is $0.003$, which is
about $25\times$ smaller than the collision radius $r_{\min}$. Any
...
comparator, PPO ends up with mean final $\lambda_2 = 0.34$ versus
$0.30$ for the constrained centroid and $0.27$ for random
velocities, and it wins on $6$ out of the $10$ seeds. PPO spreads
...
is larger, which is the qualitative behavior we wanted from a
position-controlled consensus swarm.
...
geometric swarm regime beat a collision-respecting centroid
heuristic while preserving inter-agent spacing. The
